...
def generate_address():
    ...
    access_token = ''.join(
        [random.choice(string.ascii_lowercase[:21]) for _ in range(20)])
    ...
    response_text = resp.content.decode('utf-8-sig').lstrip('\ufeff')
    email = json.loads(response_text)['email']
...
def fetch_mails(email, access_token):
    try:
        ...
        resp = requests.get(base_url + '/index/refresh', headers=headers)
        response_json = json.loads(
            resp.content.decode('utf-8-sig').lstrip('\ufeff'))

        new_list = []
        for i in list(response_json):
            mail = {'id': i['id'], 'subject': i['predmet'],
                    'sender': i['od'], 'time': i['kdy'], 'content': i['akce']}
...
def force_extend_inbox(time: ExtendTime, email, access_token):
    try:
        ...
        requests.get(base_url + '/expirace/' +
                     str(time.value), headers=headers)
...
def fetch_timings(email, access_token):
    try:
        ...
        headers["X-Requested-With"] = "XMLHttpRequest"
        headers["Connection"] = "close"
        headers["Cookie"] = f"PHPSESSID={access_token}; TMA={email}"

        resp = requests.get(base_url + '/index/zivot', headers=headers)
        response_json = json.loads(
            resp.content.decode('utf-8-sig').lstrip('\ufeff'))

        print('heyy:' + str(response_json))

        return {'creation_time': response_json['ted'], 'expire_time': response_json['konec']}

    except (requests.JSONDecodeError):
        raise TempMailException('The mail inbox is invalid or expired.')


def get_content(email, access_token, id):
    try:
        headers = {}
        headers["Accept"] = "application/json"
        headers["X-Requested-With"] = "XMLHttpRequest"
        ...
        resp = requests.get(base_url + f'/email/id/{id}', headers=headers)
...
class TempMailInbox():
    ...
    def get_all_mails(self) -> list[TempMail]:
        ...
        for json_mail in json_mails:
            ...
            temp_mail.sender_address = ""
            temp_mail.sender_name = ""
    ...
    """
        links = get_content(self.address, self.access_token, id)
        return links
        # mails = self.get_all_mails()
